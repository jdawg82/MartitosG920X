 ...
 */
static ssize_t diff_show(struct device *dev,
...
static ssize_t diff_dump(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%du", &diff);
	return count;
}
static DEVICE_ATTR(diff, (S_IWUSR|S_IRUGO), diff_show, diff_dump);
struct kobject *diff_kobj;
EXPORT_SYMBOL_GPL(diff_kobj);
/* soc should be 0.1% unit */
static int max77843_fg_read_soc(struct max77843_fuelgauge_data *fuelgauge)
{
	...
	t1 = ((data[1] * 100) + (data[0] * 100 / 256)) / 10;
	t2 = max77843_fg_read_vfsoc(fuelgauge);
	temp = t2;
	

	psy_do_property("battery", get, POWER_SUPPLY_PROP_STATUS, value);

	if ((value.intval == POWER_SUPPLY_STATUS_DISCHARGING) && (fuelgauge->capacity_old <=60)) {
		if(calibrate_battery==false){
 		 calibrate_battery=true;
		 pr_info("%s : MARTITOS: SET CALIBRATION FLAG TO TRUE\n", __func__);
		}
	}
	if (value.intval == POWER_SUPPLY_STATUS_FULL) {
		if(t1>t2){
    		  diff = t1-t2;
		  pr_info("%s : MARTITOS: SETTED GLOBAL DIFF!!!!\n", __func__);
		}
	}
	if ((value.intval == POWER_SUPPLY_STATUS_DISCHARGING) && (calibrate_battery==true) && (fuelgauge->capacity_old == 95)) {
		  calibrate_battery=false;
		  if(max77843_fg_reset(fuelgauge)){
		  pr_info("%s : MARTITOS: CALIBRATED!!!!\n", __func__);
		  }else{
		  pr_info("%s : MARTITOS: CALIBRATION FAIL\n", __func__);
		}
	}else if (value.intval == POWER_SUPPLY_STATUS_DISCHARGING) {
	      if(diff == fuelgauge->capacity_old){
		diff = diff - 1;
	      }
	}
...
bool max77843_fg_init(struct max77843_fuelgauge_data *fuelgauge)
{
	...
	u8 data[2] = {0, 0};
	int rc = 0;
	diff_kobj = kobject_create_and_add("martitos_gauge", NULL) ;
	if (diff_kobj == NULL) {
		pr_warn("%s: martitos_guauge create_and_add failed\n", __func__);
	}
	rc = sysfs_create_file(diff_kobj, &dev_attr_diff.attr);
	if (rc) {
		pr_warn("%s: sysfs_create_file failed for diff_martitos\n", __func__);
	}
