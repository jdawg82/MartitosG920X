...
static int max77843_fg_get_property(struct power_supply *psy,
    ...
    struct max77843_fuelgauge_data *fuelgauge =
    container_of(psy,
    struct max77843_fuelgauge_data, psy_fg);
...
static void max77843_fg_isr_work(struct work_struct *work) {
    struct max77843_fuelgauge_data *fuelgauge =
    container_of(work,
    struct max77843_fuelgauge_data, isr_work.work);
...
max77843_fuelgauge_probe(struct platform_device *pdev) {
    ...
    if (reg_data >= 990 && reg_data <= 1000 && reg_data != fuelgauge->capacity_max) {
